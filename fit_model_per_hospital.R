...
hospitalizaciones_estatal <- readRDS("processed/HospitalizacionesMX_estatal.rds")
hospitalizaciones_unidad  <- readRDS("processed/HospitalizacionesMX_unidad_medica.rds")

#Media móvil por entidad federativa
estados_clave <- sort(unique(hospitalizaciones_estatal$Estado))
estados_clave <- tibble(Estado = estados_clave, EstadoID = 1:length(estados_clave))

clues_clave <- sort(unique(hospitalizaciones_unidad$CLUES))
clues_clave <- tibble(CLUES = clues_clave, CLUES_ID = 1:length(clues_clave))


message("Procesando hospitalizados por estado")
tic()
observados <- hospitalizaciones_estatal %>% 
  ...
  ungroup() %>%
  left_join(estados_clave, by = "Estado") %>%
  select(-Estado) %>%
  identity()
toc()

#Para cada unidad médica ~ 15 minutos
message("Procesando hospitalizados por unidad médica")
tic()
observados_UM <- hospitalizaciones_unidad %>%
  select(-`Ventilación (%)`,-`UCI y Ventilación (%)`, -`Institución`,`Unidad médica`) %>%
  mutate(`Hospitalizados (%)` = `Hospitalizados (%)`/100) %>%
  mutate(Fecha = ymd(Fecha)) %>%
  filter(!is.na(CLUES)) %>%
  arrange(CLUES, Fecha) %>%
  group_by(Fecha, CLUES) %>%
  mutate(`Hospitalizados (%)` = 
           rollapply(`Hospitalizados (%)`, width = 7, 
                     FUN = function(x) mean(x, na.rm=TRUE), partial=TRUE, 
                     fill = NA, align="left")) %>%
  rename(`Hospitalizados CLUES` = `Hospitalizados (%)`) %>%
  filter(Fecha > ymd("2020/04/10")) %>% 
  ungroup() %>%
  left_join(estados_clave, by = "Estado") %>%
  left_join(clues_clave, by = "CLUES") %>%
  select(-Estado,-CLUES) %>%
  mutate(`Hospitalizados CLUES` = if_else(`Hospitalizados CLUES` < 0, 0, `Hospitalizados CLUES`)) %>%
  mutate(`Hospitalizados CLUES` = if_else(`Hospitalizados CLUES` > 1, 1, `Hospitalizados CLUES`)) %>%
  identity() 
toc()

#OJO ALGUNOS LOS HAN DEJADO DE REPORTAR e.g. el 243  o el 93
experimento <- observados_UM %>% filter(CLUES_ID == 249)

#Mínimo y máximo de fechas.
min_fecha     <- min(observados_UM$Fecha)
max_fecha     <- max(observados_UM$Fecha)
