...
//     \sum_{i=1}^r [b_{i,j} * cos(2*pi*t/(365/i)) + a_{i,j} * sin(2*pi*t/(365/i))] 
//     \sum_{k=1}^p \lambda_k * PHosp_{j,t-k} +  \sum_{k=1}^q \phi_k * error_{j,t-k} 
//
// donde PHosp_{j,t} ~ Beta(logit(PHosp_{j,t}), kappa). Esto corresponde a una 
// estructura ARIMA(p,q) sobre logit(PHosp_{j,t}).
...
data {
  ...
  int<lower=0> hmin;                // Cantidad mínima de semanas de retraso del semáforo
  int<lower=0> hmax;                // Cantidad máxima de semanas de retraso del semáforo
  matrix[nestados, ndias] PHosp;    // % Ocupación hospitalaria al día t para estado j
  matrix[nestados, ndias + dias_predict] rojo;   //Matriz indicadora semáforo rojo
  matrix[nestados, ndias + dias_predict] naranja; //Matriz indicadora semáforo naranja
  matrix[nestados, ndias + dias_predict] amarillo; //Matriz indicadora semáforo amarillo
...
parameters {
  ...
  //Efectos del semáforo
  //----------------------------------------------------------------------------
  real mu_semaforo;
  real<lower=0> sigma_semaforo;
  ...
  //Términos del efecto del semáforo
  //----------------------------------------------------------------------------
  real<lower=0> brojo[hmax - hmin + 1];
  real<lower=0> bnaranja[hmax - hmin + 1];
  real<lower=0> bamarillo[hmax - hmin + 1];
...
model {
  ...
  mu_time        ~ normal(mu_mu_hiper, sigma_mu_hiper);
  sigma_time     ~ cauchy(0, sigma_mu_hiper);
  mu_semaforo    ~ normal(mu_mu_hiper, sigma_mu_hiper);
  sigma_semaforo ~ cauchy(0, sigma_mu_hiper);

  // Creamos los parámetros
  lambda ~ normal(mu, sigma);
  phi    ~ normal(mu_phi_prior, sigma_phi_prior);
  alpha  ~ normal(mu_estado, sigma_estado);
  kappa  ~ cauchy(mu_kappa_prior, sigma_kappa_hiper);
  
  brojo      ~ normal(mu_semaforo, sigma_semaforo);
  bnaranja   ~ normal(mu_semaforo, sigma_semaforo);
  bamarillo  ~ normal(mu_semaforo, sigma_semaforo);
  ...
  for (t in (p + 1):(p + q)){
    ...
    for (i in hmin:hmax){
      if (t - 7*i > 0){
      logit_p_estado[,t] += -(brojo[i - hmin + 1] + bnaranja[i - hmin + 1] + bamarillo[i - hmin + 1])*rojo[,t - 7*i] 
                            -(bnaranja[i - hmin + 1] + bamarillo[i - hmin + 1])*naranja[,t - 7*i]
                            -(bamarillo[i - hmin + 1])*amarillo[,t - 7*i];
    }
    }
...
generated quantities {
  ...
  vector[hmax - hmin + 1] eta_rojo;
  vector[hmax - hmin + 1] eta_naranja;
  vector[hmax - hmin + 1] eta_amarillo;
  
  //Final sums
  eta_rojo     = to_vector(brojo) + to_vector(bnaranja) + to_vector(bamarillo);
  eta_naranja  = to_vector(bnaranja) + to_vector(bamarillo);
  eta_amarillo = to_vector(bamarillo);
  ...
  for (t in ((p + q) + 1):ndias){
    ...
    for (k in 1:q){
      logit_p_estado[,t] += (phi[k]*(logit_p_estado[,t - k] - logit(PHosp[,t - k]))).*to_vector( normal_rng(rep_vector(0.0, nestados), sigma_error) );
    }
    for (i in hmin:hmax){
      if (t - 7*i > 0){
      logit_p_estado[,t] += -(brojo[i - hmin + 1] + bnaranja[i - hmin + 1] + bamarillo[i - hmin + 1])*rojo[,t - 7*i] 
  ...
  for (t in (ndias + 1):(ndias + p)){
    ...
    for (i in hmin:hmax){
      logit_p_estado[,t] += -brojo*rojo[,t - 7*i] 
                            -bnaranja*naranja[,t - 7*i]
                            -bamarillo*amarillo[,t - 7*i]
                            -bverde*verde[,t - 7*i];
    }
    HospPred[, t] = to_vector( beta_proportion_rng(inv_logit(logit_p_estado[,t]), kappa) );
  }
