...
data {
  ...
  matrix[nestados, ndias + dias_predict] verde; //Matriz indicadora semáforo verde
...
parameters {
  ...
  real<lower=0> bverde[hmax - hmin + 1];
...
model {
  ...
  bverde     ~ normal(mu_semaforo, sigma_semaforo);
  ...
  for (t in (p + 1):(p + q)){
    ...
    for (i in hmin:hmax){
      if (t - 7*i > 0){
      logit_p_estado[,t] += -(brojo[i - hmin + 1] + bnaranja[i - hmin + 1] + bamarillo[i - hmin + 1] + bverde[i - hmin + 1])*rojo[,t - 7*i] 
                            -(bnaranja[i - hmin + 1] + bamarillo[i - hmin + 1] + bverde[i - hmin + 1])*naranja[,t - 7*i]
                            -(bamarillo[i - hmin + 1] + bverde[i - hmin + 1])*amarillo[,t - 7*i]
                            -bverde[i - hmin + 1]*verde[,t - 7*i];
    }
    }
