...
if len(argv) == 3:
    ...
    files_downloaded = os.listdir("raw/estatal")
    files_downloaded = [e.replace("UCI_", "").replace(".csv", "") for e in files_downloaded if e.startswith('UCI_')]
...
if datetime.strptime(descargar_desde,'%Y-%m-%d') <= datetime.strptime(descargar_hasta,'%Y-%m-%d'):
    ...
    print("Descargando datos estatales desde " + descargar_desde +  " hasta " + descargar_hasta)
        ...
        # --------------------------------
            if text_found:
                ...
                if estatal:
                    ...
                    time.sleep(sleep_time)
                    browser.find_element('xpath',"/html/body/section/section[2]/div[2]/section/article[2]/article/div[1]/div/div[2]/div/div[1]/button[1]/span").click()
                else:
